...
procedure DetectGraph(var driver,mode:smallint);
...
procedure GetModeRange(driver:smallint; var width,height:smallint);
...
procedure GetAnim(x1,y1,x2,y2:smallint; color:longword; var anim:AnimatType);
...
{ExtraCommands}
procedure PutSubImage(x1,y1,SubWidth,SubHeight,SubX,SubY:smallint; Scale:real; var Spritesheet; bit:word);
{EndExtraCommands}
...
procedure GetRGBComponents(color:longword; var r,g,b:word);
...
procedure GetFontSettings(var fontname:shortstring; var width,height:word; var ttfont:boolean);
...
begin
  if palExist then MapColor:=PALETTEINDEX(color mod palSize)
              else MapColor:=color;
end;
...
begin
  ...
  case mess of
    ...
    WM_SYSKEYDOWN: begin
                     if Assigned(KeyboardHook) then
                       WinGraphProc:=KeyboardHook(grHandle,mess,wParam,lParam);
                     Exit;
                   end;
    ...
    WM_MBUTTONUP: begin
                    if Assigned(MouseHook) then
                      WinGraphProc:=MouseHook(grHandle,mess,wParam,lParam);
                    Exit;
                  end;
    WM_CLOSE: begin
                grCloseRequest:=true;
  ...
  WinGraphProc:=DefWindowProc(grHandle,mess,wParam,lParam);
end;
...
function CloseGraphRequest: boolean;
begin
  CloseGraphRequest:=grCloseRequest;
end;
...
begin
  case grDriver of
    D1bit    : GetDriverName:='D1bit - 2 colors';
    D4bit    : GetDriverName:='D4bit - 16 colors';
    D8bit    : GetDriverName:='D8bit - 256 colors';
    NoPalette: GetDriverName:='NoPalette - all colors';
  else
    GetDriverName:='no graphics driver';
  end;
end;

function GetGraphMode: smallint;
begin
  if (grDriver >= 0) then GetGraphMode:=grMode else GetGraphMode:=-1;
end;
...
begin
  case mode of
    m320x200  : GetModeName:='320 x 200';
    m640x200  : GetModeName:='640 x 200';
    m640x350  : GetModeName:='640 x 350';
    m640x480  : GetModeName:='640 x 480';
    m720x350  : GetModeName:='720 x 350';
    m800x600  : GetModeName:='800 x 600';
    m1024x768 : GetModeName:='1024 x 768';
    m1280x1024: GetModeName:='1280 x 1024';
    mDefault  : GetModeName:='Windows default';
    mMaximized: GetModeName:='Maximized';
    mFullScr  : GetModeName:='Full screen';
    mCustom   : GetModeName:='Custom';
  else
    GetModeName:='invalid graphics mode';
    Exit;
  end;
  GetModeName:=GetModeName+' (VESA)';
end;
...
begin
  case errorcode of
    ...
    grInvalidDriver  : GraphErrorMsg:='Invalid graphics driver';
    grInvalidMode    : GraphErrorMsg:='Invalid graphics mode';
    grNotWindow      : GraphErrorMsg:='Creation of graphics window failed';
    grNoInitGraph    : GraphErrorMsg:='Graphics window not initialized. Use InitGraph';
    grInvalidFont    : GraphErrorMsg:='Invalid font selection';
    grInvalidFontNum : GraphErrorMsg:='Invalid font number';
    grInvalidParam   : GraphErrorMsg:='Invalid parameter value';
    grNoPalette      : GraphErrorMsg:='No palette available. Change graphics driver';
    grNoOpenGL       : GraphErrorMsg:='OpenGL driver not initialized';
    grError          : GraphErrorMsg:='General graphics error';
  else
    GraphErrorMsg:='Unrecognized error code';
  end;
  GraphErrorMsg:='Error: '+GraphErrorMsg;
end;

function GraphResult: smallint;
begin
  GraphResult:=grResult; grResult:=grOk;
end;
...
begin
  ...
  begin
    ...
    case bit of
      BkgPut: begin
                if grDirect then BitBlt(grWindow,x1,y1,bmWidth,bmHeight,grTemp,0,0,SRCCOPY);
...
begin
  ...
  begin
    ...
    EnterCriticalSection(protect_devices);
    SetDIBits(grWindow,hbmp,0,biHeight,@img.bmiBits,PBITMAPINFO(@img.bmiInfoHeader)^,DIB_RGB_COLORS);
    SelectObject(grTemp,hbmp);
...
{Added by thomasBuckland 25/11/2018}
procedure PutSubImage(x1,y1,SubWidth,SubHeight,SubX,SubY:smallint; Scale:real; var Spritesheet; bit:word);
var Sprite,Map: HBITMAP;
	SpriteDC: HDC;
	ofx,ofy:longint;
    img : TImage absolute Spritesheet;
    rop : DWORD;
begin
  grResult:=grOK;
  if not(grEnabled) then begin
  ...
  begin
    ...
    Map:=CreateCompatibleBitmap(grMemory,biWidth,biHeight);
	Sprite:=CreateCompatibleBitmap(grMemory,SubWidth,SubHeight);
	SpriteDC := CreateCompatibleDC(0);
    EnterCriticalSection(protect_devices);
    SetDIBits(grWindow,Map,0,biHeight,@img.bmiBits,PBITMAPINFO(@img.bmiInfoHeader)^,DIB_RGB_COLORS);
    SelectObject(grTemp,Map);
	SelectObject(SpriteDC,Sprite);
	BitBlt(SpriteDC,0,0,SubWidth,SubHeight,grTemp,ofx,ofy,SRCCOPY);
	if grDirect then StretchBlt(grWindow,x1,y1,Round(SubWidth*Scale),Round(SubHeight*Scale),SpriteDC,0,0,SubWidth,SubHeight,rop);
	StretchBlt(grMemory,x1,y1,Round(SubWidth*Scale),Round(SubHeight*Scale),SpriteDC,0,0,SubWidth,SubHeight,rop);
...
function GetMaxColor: longword;
begin
  if palExist then GetMaxColor:=palSize-1 else GetMaxColor:=$FFFFFF;
end;
...
function GetPaletteSize: smallint;
begin
  if palExist then GetPaletteSize:=palSize else GetPaletteSize:=-1;
end;
...
begin
  ...
  EnterCriticalSection(protect_devices);
  GetPixel:=windows.GetPixel(grMemory,x+origX,y+origY);
  LeaveCriticalSection(protect_devices);
  if palExist then GetPixel:=GetNearestPaletteIndex(grPalette,GetPixel);
end;

function GetRGBColor(r,g,b:word): longword;
begin
  GetRGBColor:=RGB(r,g,b);
  if palExist then GetRGBColor:=GetNearestPaletteIndex(grPalette,GetRGBColor);
end;
...
begin
  ...
  LeaveCriticalSection(protect_devices);
  if (InstallUserFont = -1) then grResult:=grInvalidFont;
end;
